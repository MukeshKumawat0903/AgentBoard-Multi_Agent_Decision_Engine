...
Emits SSE events via an optional queue_list / replay_buffer for live streaming.
"""
...
class DebateController:
    ...
    Pass ``queue_list`` and ``replay_buffer`` (mutable lists owned by the
    dependency layer) to enable SSE broadcasting for connected stream clients.
    When omitted the controller runs silently (backward-compatible sync mode).
    """
    ...
    def __init__(
        ...
        # SSE event channels – set to live lists managed by dependencies.py
        self._queue_list: list | None = queue_list
        self._replay_buffer: list | None = replay_buffer
    ...
    def _emit(self, event_type: str, data: dict) -> None:
        """Broadcast a typed event to all connected SSE clients (non-blocking)."""
        payload = {"type": event_type, **data}
        if self._replay_buffer is not None:
            self._replay_buffer.append(payload)
        if self._queue_list:
            for q in list(self._queue_list):  # snapshot avoids mutation races
    ...
    async def execute(self) -> FinalDecision:
        ...
        while True:
            ...
            self._emit("phase_started", {"round_number": state.current_round, "phase": "proposal"})
            await self._run_proposals(state)

            # Phase 2 – Cross-examination
            round_data.phase = "critique"
            self._emit("phase_started", {"round_number": state.current_round, "phase": "critique"})
            await self._run_cross_examination(state)

            # Phase 3 – Revisions
            round_data.phase = "revision"
            self._emit("phase_started", {"round_number": state.current_round, "phase": "revision"})
            await self._run_revisions(state)

            # Phase 4 – Convergence check
            round_data.phase = "convergence"
            self._emit("phase_started", {"round_number": state.current_round, "phase": "convergence"})
